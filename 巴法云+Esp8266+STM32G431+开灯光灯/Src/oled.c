...
{  
    ...
    if(line < 8)
    {
        OLED_WR_Byte (0xb0+line, OLED_CMD); // è®¾ç½®é¡µåœ°å€ä¸ºè¦æ¸…é™¤çš„è¡Œ
        OLED_WR_Byte (0x00, OLED_CMD);      // è®¾ç½®åˆ—åœ°å€ä½ä½ä¸º0
        OLED_WR_Byte (0x10, OLED_CMD);      // è®¾ç½®åˆ—åœ°å€é«˜ä½ä¸º0
        for(n=0; n<128; n++)
        {
            OLED_WR_Byte(0, OLED_DATA); // å‘è¯¥è¡Œçš„æ¯ä¸ªåƒç´ å‘é€æ¸…é›¶æ•°æ®
        }
    }
}
